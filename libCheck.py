...
def formatF_Check_Missing_In_Folder(strPar, iStartPar, iStopPar, nSum):
  return f"In {strPar} (spanning {iStopPar-iStartPar} rows ({iStartPar}-{iStopPar-1})), {nSum} files are missing.\n"
...
def checkSummarizeMissingInterior(fsMeta, fsDir):
...
def checkHash(fsMeta, fsDir, iStart):
  tStart=time.time()
    ...
    if(strHash!=strHashOld):  # If hashes mismatches
      ...
      if(not boTMatch or not boSizeMatch ): # If meta data mismatches
        ...
        if(strBase==leafFileMetaOld):
          ...
          StrTmp.append(ANSI_FONT_BOLD+strLab+" MISMATCH!!!"+ANSI_FONT_CLEAR+" (remember run HASHBERT SYNC first)")
  ...
  if(nMisMatchTimeSize): myConsole.printNL(ANSI_FONT_BOLD+"META (SIZE/MTIME) MISMATCHES!!!"+ANSI_FONT_CLEAR+" (on "+str(nMisMatchTimeSize)+" files) This happens when HASHBERT SYNC wasn't called before checking (So call HASHBERT SYNC before running HASHBERT CHECK)")
